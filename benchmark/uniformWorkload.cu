...
generateWorkloadLargeKey(size_t keySize, int size, int n, unsigned *seed,
    ...
    for (int i = 0; i < size; i++) {
        if (rand_r(seed) % 100 < ratioOfReads) {
            vec.push_back({(unsigned long long) (rand_r(seed) % n) + 1, 0, nullptr, REQUEST_GET});
        } else {
            if (rand_r(seed) % 100 < 50) {
                vec.push_back({(unsigned long long) (rand_r(seed) % n) + 1, 0, new data_t(keySize),
                               REQUEST_INSERT});
            } else {
                vec.push_back(
                        {(unsigned long long) (rand_r(seed) % n) + 1, 0, nullptr, REQUEST_REMOVE});
            }
        }
