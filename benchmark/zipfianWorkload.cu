...
generateWorkloadZipfLargeKey(size_t keySize, int size, double theta, int n, double zetaN, unsigned *seed,
    ...
    for (int i = 0; i < size; i++) {
        if (rand_r(seed) % 100 < ratioOfReads) {
            vec.push_back({(unsigned long long) cse498::rand::rand_zipf_r(seed, n, zetaN, theta), 0, nullptr, REQUEST_GET});
        } else {
            if (rand_r(seed) % 100 < 50) {
                vec.push_back({(unsigned long long) cse498::rand::rand_zipf_r(seed, n, zetaN, theta), 0, new data_t(keySize),
                               REQUEST_INSERT});
            } else {
                vec.push_back(
                        {(unsigned long long) cse498::rand::rand_zipf_r(seed, n, zetaN, theta), 0, nullptr, REQUEST_REMOVE});
            }
        }
